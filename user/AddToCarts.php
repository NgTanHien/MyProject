...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        $updateQuery = "UPDATE giohang SET soluong = soluong + $soluong WHERE id_nguoidung = ? AND id_monan = ?";
        $stmt = $connection->prepare($updateQuery);
        $stmt->bind_param("ii", $id_nguoidung, $id_monan);
        $stmt->execute();
    } else {
        // Nếu chưa có thì thêm mới
        $insertQuery = "INSERT INTO giohang (id_nguoidung, id_monan, soluong, ngaythem) VALUES (?, ?, $soluong, NOW())";
